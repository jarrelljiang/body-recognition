...
onMounted(() => {
    ...
    function endEstimateSegmentationStats(time) {
        ...
        if (
            endInferenceTime - time.lastPanelUpdate >=
            panelUpdateMilliseconds
        ) {
            const averageInferenceTime =
                time.inferenceTimeSum / time.numInferences;
                ...
                if (segmenter.segmentPeople != null) {
                    segmentation = await segmenter.segmentPeople(camera.video, {
                        ...
                        segmentationThreshold:
                            STATE.visualization.foregroundThreshold,
                    });
                } else {
            ...
            if (fpsDisplayMode === "model") {
                ...
                if (backend === "tfjs") {
                    for (const value of segmentation) {
                        ...
                        const buffer =
                            webGLBackend.gpgpu.createBufferFromTexture(
                                res.texture,
                                1,
                                1
                            );
                        webGLBackend.gpgpu.downloadFloat32MatrixFromBuffer(
                            buffer,
                            1
                        );
